...
export const fetchCities = async () => {
  try {
    ...
    const response = await axiosInstanceCpt.get("/properties");
    console.log(
      "Fetching cities, boroughs, and postcodes data from:",
      "/properties"
    );

    // Extract properties from the response data
    const properties = response.data;

    // Create an array of cities and boroughs with their latitude and longitude
    const cityBoroughData = properties.posts
      .map((property) => [
        {
          name: property.city,
      ...
      .flat() // Flatten the array to have city and borough pairs as individual objects
      .filter(
        (location) => location.name && location.latitude && location.longitude
      ); // Ensure name, latitude, and longitude are present

    // Create an object with city and borough names as keys and latitude/longitude as values
    const citiesMap = cityBoroughData.reduce(
      (acc, { name, latitude, longitude }) => {
        // Only add the name to the map if it doesn't already exist
        if (!acc[name]) {
          acc[name] = {
            latitude,
            longitude,
    ...
    console.log("Cities and boroughs with latitude and longitude:", citiesMap);
    return citiesMap; // Return the object with city and borough names as keys and lat/long values
  } catch (error) {
    console.error(
      "Error fetching cities, boroughs",
      error.response ? error.response.data : error
    );
    throw error;
  }

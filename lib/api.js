...
export const fetchPostIdBySlug = async (slug, postSlug) => {
  try {
    // Request posts by the slug, filtering by post slug
    const response = await axiosInstance.get(`/${slug}?slug=${postSlug}`);
    const post = response.data[0]; // Assuming the API returns an array of posts

...
export const fetchImageData = async (ids) => {
  try {
    ...
    return imageData; // Return array of image data
  } catch (error) {
...
export const fetchCptPostsForSimilarPropertiesSlider = async ({
  ...
  try {
    ...
    console.log("Fetching posts from test:", url); // For debugging
...
export const fetchPostsByCategory = async (cpt, category) => {
  try {
    const response = await axiosInstanceMain.get(`${cpt}?category=${category}`);
...
export const fetchPostById = async (slug, postId) => {
  try {
    const response = await axiosInstanceMain.get(`/${slug}/${postId}`);
...
export const fetchCptNames = async () => {
  try {
    const response = await axiosInstanceMain.get(`/types`);
